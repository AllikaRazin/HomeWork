...
namespace BinaryTree.Ext
{
    public static class TreeTasksExt
    {
        ...
        public static IEnumerable<Task10_LevelResult> Task10_CountNodsAndSheetsByLevels<T>(this Tree<T> tree) where T : IComparable<T> =>
          from nodes in tree.GetByLevelsEnumerable()
          select new Task10_LevelResult
          {
              CountNods = nodes.Count(x => x.Left != null || x.Right != null),
              CountSheets = nodes.Count(x => x.Left == null && x.Right == null)
          };
        
        public class Task10_LevelResult
        {
            public int CountNods { get; set; }
            public int CountSheets { get; set; }
            public override bool Equals(object obj) => obj is Task10_LevelResult target && target.CountNods == CountNods && target.CountSheets == CountSheets;
